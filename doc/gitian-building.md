...
*Setup instructions for a gitian build of YUCOIN using a Debian VM or physical system.*
...
- [Building YUCOIN](#building-yucoin)
...
Clone the git repositories for yucoin and gitian and then checkout the yucoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the yucoin repository
...
To build YUCOIN (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the yucoin repository.
...
```bash
    ...
    From https://github.com/yucoin/yucoin
...
For example:
```bash
URL=https://github.com/konez2k/yucoin.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/yucoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/yucoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/yucoin-build.assert
```
...
[yucoin/gitian.sigs](https://github.com/yucoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
